...
import { type SettingItemType, modelList, voiceList } from './SettingPanel';
  ...
  return (
    <div className="space-y-2">
      {list.map((item) => {
        ...
        if (item.value === 'voiceModel' && !value) {
        ...
        return (
          <SettingItem
            ...
            item={item}
            onChange={(value) => {
              updateChat(chat.id, { [item.value]: value });
            }}
            value={value}
          />
        );
